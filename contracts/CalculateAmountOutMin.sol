...
contract CalculateAmountOutMin is OwnershipByAccessControl {
    ...
    function setSlippageFactor(bytes16 slippageFactor_)
        external
    ...
    function _calculateLastExecutedPrice()
        private
        view
        returns (bytes16, uint32)
    {
        ...
        return (
            ABDKMathQuad.fromUInt(uint256(reserve1)).div(
                ABDKMathQuad.fromUInt(uint256(reserve0))
            ),
            blockTimestampLast
        );
    }

    function _calculateAmountOutMin(address[] memory path, uint256 amount)
        public
        view
        returns (uint256)
    {
        ...
        return
            ABDKMathQuad.toUInt(
                ABDKMathQuad.fromUInt(amountsOut[1]).mul(_slippageFactor)
            );
    }

    function _getRouter(address router)
        private
        pure
